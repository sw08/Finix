...
class Stock(commands.Cog, name="주식"):
    ...
    async def on_ready(self):
        if not isdir("stocks"):
            makedirs("stocks")
        if not isdir("stocks/users"):
            makedirs("stocks/users")
        if not isfile("stocks/stocks.bin"):
            prices = {
                "삼송증권": {"price": 5000, "change": 0},
                "알지전자": {"price": 5000, "change": 0},
                "현내보험": {"price": 5000, "change": 0},
                "동영화학": {"price": 5000, "change": 0},
                "한와금융": {"price": 5000, "change": 0},
                "로데관광": {"price": 5000, "change": 0},
                "동안출판": {"price": 5000, "change": 0},
                "하태식품": {"price": 5000, "change": 0},
                "소울우유": {"price": 5000, "change": 0},
                "남영유업": {"price": 5000, "change": 0},
                "대환통운": {"price": 5000, "change": 0},
            }
        ...
        for i in listdir("stocks/users"):
            ...
            for i in data:
                ...
                writedata(
                    ...
                    item="point",
                    value=str(
                        int(getdata(id=user_id, item="point"))
                        + round(stock_price[i]["price"] * 0.01 * count)
                    ),
                )
        await log(
            embed=discord.Embed(
                title="정상적으로 배당금 부여", description=str(datetime.now()), color=embedcolor
            ),
            bot=self.bot,
        )
    ...
    async def _chart_stock(self, ctx):
        ...
        for i in data:
            if data[i]["change"] > 0:
                stocks.append(
                    f'+ {data[i]["price"]}(▲ {self.addzero(abs(data[i]["change"]))}) : {i}'
                )
        ...
        await sendEmbed(
            ctx=ctx, title="차트", content=f"```diff\n{stocks}```\n`바뀔때까지 남은 시간: {time}초`"
        )

    @stocks.command(
        name="매수", aliases=["구매", "ㅁㅅ", "buy"], help="주식을 삽니다", usage="[회사] [개수]"
    )
    @can_use()
    @commands.cooldown(1.0, 3, commands.BucketType.user)
    async def _buy_stock(self, ctx, name, count="모두"):
        ...
        if count in ["모두", "ㅇㅇ", "올인", "최대"]:
            count = (
                int(getdata(id=ctx.author.id, item="point")) // stocks[name]["price"]
            )
            if count == 0:
            ...
            count = int(count)
            if count < 1:
                return await warn(ctx=ctx, content="살 주식의 개수를 1개 이상으로 입력해 주세요")
        if count * stocks[name]["price"] > int(getdata(id=ctx.author.id, item="point")):
            return await warn(ctx=ctx, content="돈이 부족합니다")
        if name in data:
            data[name].append({"count": count, "price": stocks[name]["price"]})
        else:
            data[name] = [{"count": count, "price": stocks[name]["price"]}]
        deliting = []
        for i in data:
            if len(data[i]) == 0:
                deliting.append(i)
        ...
        writedata(
            ...
            item="point",
            value=str(
                int(getdata(id=ctx.author.id, item="point"))
                - count * stocks[name]["price"]
            ),
        )
        await sendEmbed(ctx=ctx, title="매수", content=f"{name}의 주식을 `{count}`주 매수했습니다.")

    @stocks.command(
        name="매도", aliases=["ㅁㄷ", "판매", "sell"], help="주식을 팝니다", usage="[회사] [개수]"
    )
    @can_use()
    @commands.cooldown(1.0, 3, commands.BucketType.user)
    async def _sell_stock(self, ctx, name, count="모두"):
        ...
        if isfile(f"stocks/users/{ctx.author.id}.bin"):
            ...
            return await warn(
                ctx=ctx, content=f"{ctx.author.mention}님은 주식을 갖고 있지 않습니다."
            )
        if not name in data:
            return await warn(
                ctx=ctx, content=f"{ctx.author.mention}님은 주식을 갖고 있지 않습니다."
        ...
        if count in ["모두", "ㅇㅇ", "올인", "최대"]:
            ...
            count = int(count)
            if count < 1:
                return await warn(ctx=ctx, content="팔 주식의 개수를 1개 이상으로 입력해 주세요")
        user_stocks = 0
        for i in data[name]:
            user_stocks += i["count"]
        ...
        writedata(
            id=ctx.author.id,
            item="point",
            value=str(int(getdata(id=ctx.author.id, item="point")) + addmoney),
        )
        await sendEmbed(ctx=ctx, title="매도", content=f"{name}의 주식을 `{counts}`주 매도했습니다.")

    @stocks.command(
        name="계좌", aliases=["account", "ㄱㅈ", "가방"], help="가지고 있는 주식을 보여줍니다."
    )
    @can_use()
    @commands.cooldown(1.0, 3, commands.BucketType.user)
    async def _account_stock(self, ctx):
        if not isfile(f"stocks/users/{ctx.author.id}.bin"):
            return await warn(ctx=ctx, content="가지고 있는 주식이 없습니다")
        with open(f"stocks/users/{ctx.author.id}.bin", "rb") as f:
            data = load(f)
        if len(data) == 0:
            return await warn(ctx=ctx, content="가지고 있는 주식이 없습니다")
