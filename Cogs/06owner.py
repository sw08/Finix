...
class Owner(commands.Cog, name="관리자"):
    ...
    @commands.command(
        name="실행", aliases=["컴파일", "eval", "ㄱ"], help="소스코드를 실행합니다", usage="[소스 코드]"
    )
    @commands.is_owner()
    ...
    @commands.group(
        name="봇", help="봇 관련 명령어를 실행합니다.", usage="<밴/언밴>", invoke_without_command=True
    )
    @commands.is_owner()
    ...
    async def _ban(self, ctx, user: discord.Member, *, reason):
        ...
        await sendEmbed(
            ctx=ctx,
            title="밴",
            content=f"{user.mention}님이 {reason} 사유로 차단당하셨습니다.\n이의는 관리자 DM으로 제출해 주십시오.",
        )
        try:
            ...
            await user.send(
                embed=discord.Embed(
                    title="밴",
                    description=f"당신은 {reason}이라는 이유로 ThinkingBot로부터 차단당하셨습니다.\n이의는 관리자 DM으로 제출해 주십시오",
                    color=embedcolor,
                )
            )
        except:
            pass
        await (self.bot.get_channel(825221941090189372)).send(
            f"{user.mention}님 - 밴\n사유: ```{reason}```"
        )
        try:
            await (self.bot.get_guild(807033213003759626)).ban(user, reason=reason)
        except:
            pass
        await log(
            embed=discord.Embed(
                title="밴",
                description=f"{user.mention}님이 {reason}이라는 이유로 ThinkingBot로부터 차단당하셨습니다.\n처리자: {ctx.author.mention}\n{(str(ctx.message.created_at))[:-7]}",
                color=embedcolor,
            ),
            bot=self.bot,
        )
    ...
    @manageBot.command(
        name="언밴",
        aliases=["차단해제", "unban", "ㅇㅂ"],
        help="봇에게서 차단을 해제하는 명령어입니다.",
        usage="[유저] [이유]",
    )
    @commands.is_owner()
    async def _unban(self, ctx, user: discord.Member, *, reason):
        ...
        with open("banned.bin", "wb") as f:
            pickle.dump(ban, f)
        await sendEmbed(
            ctx=ctx, title="밴", content=f"{user.mention}님은 {reason} 사유로 차단해제 되셨습니다."
        )
        try:
            user = await user.create_dm()
            await user.send(
                embed=discord.Embed(
                    title="밴",
                    description=f"당신은 {reason}이라는 이유로 ThinkingBot로부터 차단해제 되셨습니다.",
                    color=embedcolor,
                )
            )
        except:
            pass
        try:
            await (self.bot.get_guild(807033213003759626)).unban(user, reason=reason)
        except:
            pass
        await log(
            embed=discord.Embed(
                title="밴",
                description=f"{user.mention}님이 {reason}이라는 이유로 ThinkingBot로부터 차단해제 되셨습니다.\n처리자: {ctx.author.mention}\n{(str(ctx.message.created_at))[:-7]}",
                color=embedcolor,
            ),
            bot=self.bot,
        )
    ...
    @commands.command(
        name="공지보내기", aliases=["공지발행", "post", "ㅂㅎ"], help="공지를 발행합니다.", usage="[공지 내용]"
    )
    @commands.is_owner()
    ...
    async def _addpoint(self, ctx, user: discord.Member, addpoint: int):
        ...
        await sendEmbed(
            ctx=ctx, title="관리자송금", content=f"{addpoint}원이 {user.mention}님께 보내졌습니다."
        )
        await log(
            embed=discord.Embed(
                title="관리자송금",
                description=f"{user.mention}님의 돈에 `{addpoint}`가 추가되었습니다.\n처리자: {ctx.author.mention}\n{(str(ctx.message.created_at))[:-7]}",
                color=embedcolor,
            ),
            bot=self.bot,
        )
    ...
    @commands.command(
        name="답변",
        aliases=["ㅁㅇㄷㅂ", "문의답변", "answer"],
        help="DM으로 온 문의에 답변하는 명령어입니다.",
        usage="[답변할 내용]",
    )
    @commands.is_owner()
    async def _answer(self, ctx, *, answer=None):
        ...
        if answer is not None:
            return await (
                await (self.bot.get_user(int(ctx.channel.name))).create_dm()
            ).send(f"{ctx.author.mention}: ```{answer}```")
        answer = ""
    ...
    @manageBot.command(
        name="명령어상태",
        aliases=["command_enabled", "ㅁㄹㅇㅅㅌ", "명령어활성화"],
        help="명령어의 활성화 상태를 변경합니다",
        usage="[on/off] [명령어이름]",
    )
    @commands.is_owner()
    async def _commandState(self, ctx, state, *, name):
        ...
        if command is None:
            return await warn(ctx=ctx, content="명령어를 찾을 수 없습니다")
        command.enabled = ([True, False])[(["on", "off"]).index(state.upper())]
        await sendEmbed(
            ctx=ctx, title="명령어 상태변경", content=f"`{name}` 명령어의 상태가 `{state}`로 설정되었습니다"
        )
