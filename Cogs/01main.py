...
class Main(commands.Cog, name="잡다한것"):
    ...
    async def _makefile(self, ctx, title="file.txt", *, content):
        ...
        await sendEmbed(
            ctx=ctx,
            title="완료",
            content="DM으로 파일이 전송되었습니다.\n\n만약 전송되지 않았다면 DM을 차단하지 않았는지 확인해 보세요.",
        )

    @commands.command(
        name="색깔보기",
        aliases=["색보기", "ㅅ", "color"],
        help="RGB로 입력한 색상을 보여줍니다",
        usage="[R] [G] [B]",
    )
    ...
    @commands.command(
        name="소개설정",
        aliases=["ㅅㄱㅅㅈ", "소개말설정", "introduce"],
        help="소개말을 설정합니다. 소개발을 비워 둘시 지워집니다.",
        usage="<소개말>",
    )
    ...
    async def _introduce(self, ctx, *, content=""):
        ...
        await sendEmbed(
            ctx=ctx,
            title="소개말설정",
            content=f"{ctx.author.name}님의 소개말이 `{content}`로 변경되었습니다.",
        )

    @commands.command(
        name="이모지",
        aliases=["raw", "이모티콘", "ㅇㅁㅈ"],
        help="이모지의 raw 값을 보여줍니다.",
        usage="[이모지]",
    )
    @can_use()
    ...
    async def _barcode(self, ctx, type_code, *, content):
        try:
            ...
            if type_code.lower() not in types:
                await warn(
                    ctx=ctx,
                    content=f'바코드의 타입을 찾을 수 없습니다.\n타입 목록:\n```{", ".join([i.upper() for i in types])}```',
                )
            ...
            if str(error).endswith("ISBN must start with 978 or 979."):
                return await warn(ctx=ctx, content="이 타입은 코드가 978이나 979로 시작해야 합니다")
            elif str(error).endswith("can only contain numbers."):
                return await warn(ctx=ctx, content="이 타입은 코드에 숫자만 들어갈 수 있습니다.")
            elif str(error).endswith(
                "Command raised an exception: NumberOfDigitsError:"
            ):
                return await warn(ctx=ctx, content="내용의 글자 수가 올바르지 않습니다")
            else:
                return await warn(ctx=ctx, content="저런! 에러가 발생했습니다.")

    @commands.command(
        name="첫메시지",
        aliases=["firstmessage", "ㅊㅁㅅㅈ", "처음메시지"],
        help="채널의 첫 메시지를 보여줍니다.",
        usage="<채널 멘션>",
    )
    @can_use()
    ...
    async def _channelInfo(self, ctx, channel: discord.TextChannel = None):
        ...
        await sendEmbed(
            ctx=ctx,
            title=f"{channel.name} 채널의 첫 메시지",
            content=f"[여기를 클릭하세요](https://discord.com/channels/{ctx.guild.id}/{ctx.channel.id}/{message.id})",
        )
