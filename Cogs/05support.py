...
class Support(commands.Cog, name="지원"):
    ...
    @commands.command(
        name="봇정보", aliases=["botinfo", "봇 정보", "ㅂㅈㅂ"], help="봇의 관한 정보들을 확인합니다"
    )
    @can_use()
    async def _botinfo(self, ctx):
        embed = discord.Embed(title="봇정보", color=embedcolor)
        embed.add_field(
            name="이름", value=f"{self.bot.user}, {self.bot.user.id}", inline=False
        )
        embed.add_field(
            name="현재 연결상태", value=f"{round(self.bot.latency*1000)}ms", inline=False
        )
        embed.add_field(
            name="서버/유저 수",
            value=f"{len(self.bot.guilds)}서버, 이용자 {len(self.bot.users)}명",
        )
        embed.add_field(
            name="개발자",
            value=f"{await self.bot.fetch_user(745848200195473490)}, id 745848200195473490",
            inline=False,
        )
    ...
    @commands.command(
        name="도움", aliases=["도움말", "help", "ㄷㅇ"], usage="<명령어>", help="명령어들의 도움말을 보여줍니다"
    )
    @can_use()
    async def _help(self, ctx, *, arg=None):
        command = self.bot.get_command(str(arg))
        if (command is not None) or str(
            type(command)
        ) == "discord.ext.commands.core.Group":
            embed = discord.Embed(
                title="도움말", description=f"```{command.help}```", color=embedcolor
            )
            if command.usage is None:
                ...
                usage = command.usage
            embed.add_field(
                name="사용법", value=f"{mainprefix}{arg} {usage}", inline=False
            )
            if command.aliases != []:
            ...
            for i in cogs:
                if i.get_commands() == []:
                    del cogs[cogs.index(i)]
            embed = discord.Embed(
                title=f"1/{len(cogs)+1}페이지 - 카테고리 목록",
                description="[]는 필수적인 값을, <>는 필수적이지 않은 값들을 의미합니다. 괄호들은 빼고 입력해 주세요!",
                color=embedcolor,
            )
            embed.add_field(
                name="접두사",
                value=f'{self.bot.user.name}의 접두사는 `{"`, `".join(prefix)}`입니다',
            )
            for i in cogs:
                embed.add_field(
            ...
            for i in range(len(cogs)):
                embed = discord.Embed(
                    title=f"{i+2}/{len(cogs)+1} 페이지 - {cogs[i].qualified_name}",
                    description=f"**`{cogs[i].description}`**",
                    color=embedcolor,
                )
                ...
                for i in commandList:
                    ...
                    if i.help is None:
                        embed.add_field(
                            name=f"\n**{i.name}**",
                            value=f"`{mainprefix}{i.name}{usage}`\n",
                            inline=False,
                        )
                    else:
                        embed.add_field(
            ...
            for i in range(len(helps)):
                helps[i] = helps[i].set_footer(
                    text=f"{ctx.author} | {mainprefix}도움",
                    icon_url=ctx.author.avatar_url,
                )
            page = Paginator(
                bot=self.bot,
                message=await ctx.send(embed=helps[0]),
                embeds=helps,
                only=ctx.author,
                use_extend=True,
                extended_emojis=[
                    "<:leftend:809567692692258847>",
                    "<:left:809567681652981781>",
                    "<:right:809567682164424738>",
                    "<:rightend:809567696307617863>",
                ],
            )
            await page.start()
    ...
    async def _readpost(self, ctx):
        ...
        for i in range(count):
            ...
            embeds.append(
                discord.Embed(
                    title=f"공지 - {1+i}개/{count}개",
                    description=f"{content}\n\n------------------\n[서포트 서버 들어오기](http://support.thinkingbot.kro.kr)\n[ThinkingBot 권한없이 초대](http://invite.thinkingbot.kro.kr)\n[ThinkingBot 최소권한 초대](http://invite2.thinkingbot.kro.kr)\n[ThinkingBot 깃허브](http://github.thinkingbot.kro.kr)",
                    color=embedcolor,
                ).set_footer(icon_url=writer.avatar_url, text=f"{writer} - {date}")
            )
        embeds.reverse()
        page = Paginator(
            bot=self.bot,
    ...
    @commands.command(
        name="초대",
        aliases=["ㅊㄷ", "invite", "링크"],
        help="봇과 서포트 서버, 그리고 이 서버의 초대를 보여줍니다.",
    )
    @can_use()
    async def _invite(self, ctx):
        try:
            invite = (
                "[현재 서버 초대](" + (await ctx.guild.channels[0].create_invite()).url + ")"
            )
        except:
            invite = ""
        await sendEmbed(
            ctx=ctx,
            title="초대",
            content=f"[서포트 서버 초대](http://support.thinkingbot.kro.kr)\n[ThinkingBot 권한없이 초대](http://invite.thinkingbot.kro.kr)\n[ThinkingBot 최소권한 초대](http://invite.thinkingbot.kro.kr)\n{invite}",
        )
    ...
    async def _mails(self, ctx):
        ...
        for i in mails:
            ...
            main_embed.add_field(
                name=title, value="`" + i["time"].strftime("%y.%m.%d %H:%M") + "`"
            )
        embeds = [main_embed]
        for i in mails:
            embeds.append(
                discord.Embed(
                    title=i["title"], description=i["content"], color=embedcolor
                ).set_footer(text=i["time"].strftime("%y.%m.%d %H:%M"))
            )
        page = Paginator(
            bot=self.bot,
            message=await ctx.send(embed=embeds[0]),
